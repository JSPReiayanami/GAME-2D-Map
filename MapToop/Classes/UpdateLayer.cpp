...
bool UpdateLayer::init(){
	if (Layer::init())
	{
		...
		// 重置
		MenuItemFont* itemReset = MenuItemFont::create("reset",this,menu_selector(UpdateLayer::reset));
		itemReset->setPosition(ccp(visibleSize.width/2, 50));
		menu->addChild(itemReset);
		// 获取当前版本号
		MenuItemFont* itemGetClientVersion = MenuItemFont::create("getClientVersion",this,menu_selector(UpdateLayer::getClientVersion));
...
void  UpdateLayer::getClientVersion(CCObject* pSender){
	String* msg = String::createWithFormat("current client version : %s", getAssetsManager()->getVersion().c_str());
